...
class QrCodeUtilsTest {
    ...
    @Test
    fun `encode produces whoami v1 format`() {
        val encoded = QrCodeUtils.encode(testName, testKey)
        assert(encoded.startsWith("whoami:v1:"))
    ...
    fun `decode rejects garbage input`() {
        ...
        assertNull(QrCodeUtils.decode("whoami:v1"))
        assertNull(QrCodeUtils.decode("whoami:v1:notbase64!@#:name"))
    }

    @Test
    fun `decode rejects too few parts`() {
        assertNull(QrCodeUtils.decode("whoami:v1:keyonly"))
    }
